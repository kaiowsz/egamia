...
export async function getAllImages({limit = 9, page = 1, searchQuery = ""}: GetAllImagesProps) {
    try {
        await connectToDatabase();
...
interface GetUserImagesProps {
    limit?: number;
    page: number;
    userId: string;
}

export async function getUserImages({limit = 9, page = 1, userId}: GetUserImagesProps) {
    try {
        await connectToDatabase();
